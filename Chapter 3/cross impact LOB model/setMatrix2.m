function [A,Abar,Atilde]=setMatrix2(times,q,rho,beta)
...
for i = 1:N
    for j = 1:N
        A(i,j) = (i<j)*exp(-(times(j)-times(i)) *rho)/q(j);
        Atilde(i,j) = (i<j)*exp(-(times(j)-times(i)) *(rho+beta))/q(j);
        Abar(i,j) = (i<j)*exp(-(times(j)-times(i)) *rho)/q(j)+0.5*(i==j)*exp(-(times(j)-times(i)) *rho)/q(j);
    end
end
